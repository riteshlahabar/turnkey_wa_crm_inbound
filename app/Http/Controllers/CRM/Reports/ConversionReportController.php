<?php

namespace App\Http\Controllers\CRM\Reports;

use App\Http\Controllers\Controller;
use App\Models\CRM\CrmCourse;
use App\Models\CRM\CrmFeeQuotation;
use App\Models\CRM\CrmFollowup;
use App\Models\CRM\CrmLead;
use App\Models\CRM\CrmLeadSource;
use App\Models\CRM\CrmLeadStatus;
use App\Models\CRM\CrmStandard;
use App\Models\User;

class ConversionReportController extends Controller
{
    public function index()
    {
        $admissionStatusIds = CrmLeadStatus::where('is_admission', true)->pluck('id');
        $totalLeads = CrmLead::count();
        $admissions = CrmLead::whereIn('lead_status_id', $admissionStatusIds)->count();
        $conversion = $totalLeads > 0 ? round(($admissions / $totalLeads) * 100, 2) : 0;
        return view('crm.reports.conversion.index', compact('totalLeads', 'admissions', 'conversion'));
    }
}
